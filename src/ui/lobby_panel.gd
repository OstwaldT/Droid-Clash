...
const PLAYER_LIST_MAX_H: float = 330.0
...
func _build_ui() -> void:
	...
	panel.add_theme_stylebox_override("panel",
		UITheme.make_panel_style(Vector4(36, 36, 30, 30), 8))
	...
	vbox.add_child(UITheme.make_title("DROID-CLASH", 66))
	vbox.add_child(UITheme.make_subtitle("L  O  B  B  Y", 22))
	...
	_player_list.add_theme_constant_override("separation", 12)
	...
	UITheme.apply_font(_count_label, 24, UITheme.MUTED)
	...
	UITheme.apply_font(_countdown_label, 150, UITheme.HIGHLIGHT)
	_countdown_label.set_anchors_and_offsets_preset(Control.PRESET_CENTER)
	_countdown_label.custom_minimum_size = Vector2(252, 204)
...
func _show_empty_state() -> void:
	...
	UITheme.apply_font(empty, 22, Color(0.32, 0.32, 0.42))
...
func _add_player_row(player_id: int, player_name: String) -> void:
	...
	row.add_child(UITheme.make_swatch(swatch_color, 18.0))

	# Player number
	var num_label := Label.new()
	num_label.text = "P%d" % player_id
	num_label.custom_minimum_size.x = 39
	UITheme.apply_font(num_label, 24, Color(0.55, 0.55, 0.65))
	...
	UITheme.apply_font(name_label, 27, UITheme.TEXT)
	...
	status.text = "○  WAITING"
	status.custom_minimum_size.x = 165
	status.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	UITheme.apply_font(status, 24, Color(0.40, 0.40, 0.52))
