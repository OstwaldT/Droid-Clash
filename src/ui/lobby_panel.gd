...
const PANEL_W: float = 840.0
const PLAYER_LIST_MAX_H: float = 390.0
...
func _build_ui() -> void:
	...
	panel.add_theme_stylebox_override("panel",
		UITheme.make_panel_style(Vector4(42, 42, 36, 36), 8))
	...
	vbox.add_child(UITheme.make_title("DROID-CLASH", 78))
	vbox.add_child(UITheme.make_subtitle("L  O  B  B  Y", 27))
	...
	UITheme.apply_font(_count_label, 28, UITheme.MUTED)
	...
	UITheme.apply_font(_countdown_label, 180, UITheme.HIGHLIGHT)
	_countdown_label.set_anchors_and_offsets_preset(Control.PRESET_CENTER)
	_countdown_label.custom_minimum_size = Vector2(300, 240)
...
func _show_empty_state() -> void:
	...
	UITheme.apply_font(empty, 26, Color(0.32, 0.32, 0.42))
...
func _add_player_row(player_id: int, player_name: String) -> void:
	...
	row.add_child(UITheme.make_swatch(swatch_color, 21.0))

	# Player number
	var num_label := Label.new()
	num_label.text = "P%d" % player_id
	num_label.custom_minimum_size.x = 45
	UITheme.apply_font(num_label, 30, Color(0.55, 0.55, 0.65))
	...
	UITheme.apply_font(name_label, 33, UITheme.TEXT)
	...
	status.text = "○  WAITING"
	status.custom_minimum_size.x = 195
	status.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	UITheme.apply_font(status, 28, Color(0.40, 0.40, 0.52))
