...
# Speeders are ~2 units wide; scale to fit comfortably inside a 1.2-unit hex.
const MODEL_SCALE: float = 0.50
const MODEL_Y: float     = 0.05   # hover just above the tile
...
_model_root.position.y = MODEL_Y
...
## Walk the tree and call [cb] for every MeshInstance3D found.
...
## Tint every surface of [node]'s meshes toward [color].
## Multiplies the original albedo by the player colour so shape detail is preserved.
...
## Grey out the model when the robot dies from combat damage.
func mark_dead() -> void:
if _is_dead:
return
_is_dead = true
...
## Restore the robot to a fully alive visual state (called on rematch).
func revive() -> void:
_is_dead = false
visible  = true
scale    = Vector3.ONE
...
## Fire a rocket toward [target_world_pos] from the speeder's nose.
...
## Explosion death: scatter debris then shrink model to nothing.
func explode() -> void:
if _is_dead:
return
_is_dead = true
